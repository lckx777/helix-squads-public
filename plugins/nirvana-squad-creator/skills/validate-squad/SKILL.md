...
- nome: allReferences
  tipo: array<file>
  descricao: references/*.md (agent-format, task-format, workflow-format, squad-yaml-schema,
    config-format)
  obrigatorio: true
Saida:
...
Checklist:
  ...
  - '[ ] Todos os artefatos presentes: agents/, tasks/, workflows/, config/, squad.yaml,
    README.md'
...
Error Handling:
  strategy: abort
  fallback: Se validação encontrar erros, retornar relatório FAILED ao orquestrador
    para re-execução das fases afetadas
  notification: orchestrator
Metadata:
  story: Como pipeline, preciso validar que o squad gerado está 100% correto antes
    do deploy
  version: 1.0.0
  dependencies:
  - optimizeSquad()
