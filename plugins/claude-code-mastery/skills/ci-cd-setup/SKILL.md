...
jobs:
  claude-review:
    ...
    - name: Run Claude Review
      ...
      run: "DIFF=$(git diff origin/main...HEAD)\nclaude -p \"Review these changes\
        \ for bugs, security issues, and code quality. Be concise. Changes: $DIFF\"\
        \ \\\n  --output-format text \\\n  --max-turns 10 \\\n  --model sonnet \\\n\
        \  > review-output.txt\n"
    - name: Post Review Comment
      uses: actions/github-script@v7
      with:
        script: "const fs = require('fs');\nconst review = fs.readFileSync('review-output.txt',\
...
description: '**Task ID:** ci-cd-setup'
---
